...
show_help() {
cat << EOF
Usage: ${0##*/} [-dhsu [-e EXCLUDES] [-i INCLUDES] [-o OUTFILE] [-p PAGE_SIZE] [FILE]...
...
With no FILE the default '$input_files' will be used.
    ...
    -d           debug option
    -e EXCLUDES  A comma-separated list of artifacts to filter from the serialized dependency tree, or null (default) not
                 to filter any artifacts from the dependency tree. An empty pattern segment is treated as an implicit wildcard.
    -h           display this help and exit
    -i INCLUDES  A comma-separated list of artifacts to filter the serialized dependency tree by, or null not
...
# add Maven debug option if 'd' command line arg iwas more than once
[[ $debug -gt 1 ]] && exec_mvn="$exec_mvn -X"
### CMD ARGS
...
[[ $debug -gt 0 ]] && echo "$cmd_cleanup"
eval $cmd_cleanup >> $output_file
echo '}' >> $output_file

# clean up temp. work file if debug level is lower than '2'
# @see: http://www.linuxjournal.com/content/use-bash-trap-statement-cleanup-temporary-files
[[ $debug -lt 2 ]] && trap "rm -f $temp_output_file" EXIT
